...
updates = []
avg_steps = 1
b_size = 16#256 <-product b_size*rank_approximation
cur_lr = 10**(-3.5)
reset_every_n = 100
...
rank_approximation = 16

#RTRL input, G1 = u, G2 = A 
...
if 1==1:
    ...
    for s in range(rank_approximation+1):
        ...
        vector_s_squared_norm = dot_products_vec_s[:,s:s+1,:,:]# + epsilon#b_s*1*1*1
    ...
    for s in range(rank_approximation+1):
        ...
        matrix_s_squared_norm = dot_products_s[:,s:s+1,:,:]# + epsilon#b_s*1*1*1
    ...
    A_matrices_ort_span_norms = tf.norm(A_matrices_ort_span, axis=(2,3), keepdims=True)# + epsilon
    ...
    #now compute coefficients [computations already done in for-loop]
...
# This is C=LR^T on which low-rank approx is done
...
##################################
### tensorflow svd is slow. So we will call it on python
#singular_values, coeff_ortnorm_u_h, right_singular_values = tf.svd(matrix_ort_basis)

##################################

#because tf.svd is slow in gpu and bug driven in cpu, we call it in python
if 1 == 1:
...
#first check which singular values to keep or mix (with increasing order?)
...
#to estimate quality of approx, see how much is kept, etc.    
...
#Normalize (GS above only gives orthogonal (not normal) basis)
...
#add the singular values which are kept back in
...
#Rescale singular values (back to original size)
...
#rescale u, A to have the same norm (for stability - not clear whether necessary)
u_opt_norm = tf.norm(G1_opt, axis=(2, 3), keepdims=True)# + epsilon
A_opt_norm = tf.norm(G2_opt, axis=(2, 3), keepdims=True)# + epsilon
...
if 0 == 1:
    ...
    loss2h_tile = tf.expand_dims(loss2h, axis = 1)#b_size, 1, 1, n
    loss2h_tile = tf.tile(loss2h_tile, [1, rank_approximation, 1, 1])
    loss2W = tf.matmul(loss2h_tile, G2_opt)#b_size, rank, 1, 2n
    G1_opt_transpose =  tf.transpose(G1_opt, perm = [0,1,3,2])#b_size, rank, n+vocab+1,1
    loss2W = tf.reduce_sum(G1_opt_transpose*loss2W, axis = (0,1))/b_size
...
loss2W_avg_ = tf.clip_by_norm(loss2W_avg / avg_steps, 1.)
